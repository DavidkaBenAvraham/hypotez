...
@namespace src: src
  \package src.google.spreadsheets
\file __init__.py
...
class Spreadsheet:
    ...
    def create(self, title, sheetTitle, rows = 1000, cols = 26, scenario_language = 'en_US', timeZone = 'Etc/GMT'):
        spreadsheet = self.service.spreadsheets().create(body = {
            'properties': {'title': title, 'scenario_language': scenario_language, 'timeZone': timeZone},
...
credentials = ServiceAccountCredentials.from_json_keyfile_name(GOOGLE_CREDENTIALS_FILE, ['https://www.googleapis.com/auth/spreadsheets',
                                                                                  'https://www.googleapis.com/auth/drive'])
...
def create_pricelist(docTitle,sheetTitle,values:list):
    ...
    ss = Spreadsheet(GOOGLE_CREDENTIALS_FILE, debugMode = True)
    ss.create(docTitle, sheetTitle, rows = rowCount + 3, cols = 70, scenario_language = "en_US", timeZone = "Asia/Jerusalem")
...
def testCreateTimeManagementReport():
    ...
    ss = Spreadsheet(GOOGLE_CREDENTIALS_FILE, debugMode = True)
    ss.create(docTitle, sheetTitle, rows = rowCount + 3, cols = 8, scenario_language = "ru_RU", timeZone = "Europe/Moscow")
    ss.shareWithAnybodyForWriting()
