...
class Beeper():
    ...
    @param silent   `bool` :  `True` - без звука
    """
    ...
    async def beep(level: Union[BeepLevel, str] = BeepLevel.INFO, frequency: int = 400, duration: int = 1000) -> None:
        ...
        if isinstance (level, str):
            ...
            if level == 'warning': melody = BeepLevel.WARNING.value[0]
            if level == 'debug': melody = BeepLevel.DEBUG.value[0]
            if level == 'error': melody = BeepLevel.ERROR.value[0]
            if level == 'long_error': melody = BeepLevel.LONG_ERROR.value[0]
            if level == 'critical': melody = BeepLevel.CRITICAL.value[0]
            if level == 'bell': melody = BeepLevel.BELL.value[0]
        
        elif isinstance (level, BeepLevel):
            melody = level.value[0]

        # melody = level.value[0] if isinstance(level, BeepLevel) else level
        """! @todo обработать ситуацию с `level`, если пришел не `Beep` """
