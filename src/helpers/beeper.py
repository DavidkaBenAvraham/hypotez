...
def silent_mode(func):
    """
    @brief Функция-декоратор для управления режимом "беззвучия".
    ...
    def wrapper(*args, **kwargs):
        """
        @brief Внутренняя функция-обертка для проверки режима "беззвучия" перед выполнением функции.
...
class Beeper():
    ...
    async def beep(level: Union[BeepLevel, str] = BeepLevel.INFO, frequency: int = 400, duration: int = 1000) -> None:
        """! @~russian
        @brief Звуковой сигнал оповещения 
