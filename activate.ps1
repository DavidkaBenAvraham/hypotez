...
# Полный путь к venv\Scripts\py.exe
$venvPyPath = Join-Path -Path $VenvDir -ChildPath "Scripts\py.exe"

# Проверка существования символической ссылки
if (-Not (Test-Path $venvPyPath -PathType Leaf)) {
    # Создайте ссылку, если она не существует
    $venvPythonPath = (Get-Command python.exe).Path
    New-Item -ItemType SymbolicLink -Path $venvPyPath -Target $venvPythonPath
...
$venvJupyterPath = (Get-Command jupyter-notebook.exe).Path
